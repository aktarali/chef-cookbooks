...
template "/etc/mysql/pinggrants.sql" do
  source "grantsping.sql.erb"
  variables(
...
if passwords.root_password && !passwords.root_password.empty?
  ...
  execute "mysql-install-privileges" do  # ~FC009 - `sensitive`
    command "/usr/bin/mysql -p'#{passwords.root_password}' -e '' &> /dev/null > /dev/null &> /dev/null ; if [ $? -eq 0 ] ; then /usr/bin/mysql -p'#{passwords.root_password}' < /etc/mysql/pinggrants.sql ; else /usr/bin/mysql < /etc/mysql/pinggrants.sql ; fi ;" # rubocop:disable LineLength
    action :nothing
    subscribes :run, resources("template[/etc/mysql/pinggrants.sql]"), :immediately
    sensitive true
  end
else
  # Simpler path...  just try running the grants command
  execute "mysql-install-privileges" do
    command "/usr/bin/mysql < /etc/mysql/pinggrants.sql"
    action :nothing
    subscribes :run, resources("template[/etc/mysql/pinggrants.sql]"), :immediately
    sensitive true
  end
